...
def main():
    ...
    filepath = "../data/"
    matfile = "../data/april_2021/SimpleTunnel_straightPath_5Hz.mat"
    imfile = "../simulator_matlab/straightTunnel_long.png"
    estimates, gts = bag2numpy(f'simpleTunnel_straightPath_lidar+imu+gnss.bag')
    estimatesAfterSmoothing, velocities, landmarks, biases = csv2numpy(f"{filepath}LatestRun.csv", estimates)
    run2TunnelStart = estimates.times[0]
    run1TunnelEnd = estimates.times[3]
    greyArea = []
    ...
    print(estimatesAfterSmoothing.times, gts.times[-10:])
